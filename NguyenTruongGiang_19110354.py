...
grabt_model = GradientBoostingRegressor(max_depth = 5,loss='huber',
                                  min_samples_split = 6,
                                  min_samples_leaf = 8,
                                  max_features = None,
                                  random_state = 42)
...
model_random.fit(processed_test_set_val, y_valid)
r2score_model_random_test, rmse_model_random1_test = r2score_and_rmse(model_random, processed_test_set_val, y_valid)
print('\nR2 score (on training data, best=1):',r2score_model_random_test)
print("RMSE trên tập train: ", rmse_model_random1_test.round(decimals=1))   
...
def fine_turn_random():
    ...
    if kt:
        ...
        param_grid = [
            {'bootstrap': [True], 'n_estimators': [3, 15, 30], 'max_features': [2, 12, 20, 39],'min_samples_leaf':[1,2,4,6,8]},
            {'bootstrap': [False], 'n_estimators': [3, 5, 10, 20], 'max_features': [2, 6, 10],'min_samples_leaf':[1,2,4,6,8]}]
        grid_search_random_model = GridSearchCV(model_random, param_grid, cv=5, scoring='neg_mean_squared_error', return_train_score=True, 
        refit=True)
